...
=======
    ...
    if args.spec_file:
        ...
        #SD: working here! need to normalize counts in Omega as well
        peakcounts = all_results.groupby('spec_id')['target'].apply(peakcount)
        ...
        peakcounts = all_results.groupby('spec_id')['target2'].apply(peakcount)
        peakcounts.to_csv(args.pep_file+".pctmp",index=True)
        with open(args.pep_file+".pctmp") as f:
            for row in f:
                l= row.rstrip().split(',')
