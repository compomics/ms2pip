    ...
    return dict(
        zip(
            ...
            (
                sorted(
                    get_intense_mzs(
                        np.concatenate(_mzs, axis=None), np.concatenate(_intensities, axis=None)
                    )
                )
...
class MatchSpectra:
    ...
    def _generate_peptide_list(self):
        peptides = [
            (
                spec_id,
                self.mods.calc_precursor_mz(peptide, modifications, charge)[1],
    ...
    def match_mgfs(self, mgf_files, max_error=0.02):
        ...
        for mgf_file in mgf_files:
            ...
            with pyteomics.mgf.read(
                mgf_file, use_header=False, convert_arrays=0, read_charges=False
            ) as reader:
                for spectrum in reader:
                    ...
                    while i < len(precursors) and precursors[i] < pepmass + max_error:
                        ...
                        if match_mzs(
                            sorted(spectrum["m/z array"]), pred_peaks, max_error=max_error
                        ):
    ...
    def match_sqldb(self, sqldb_uri="postgresql:///ms2pip", max_error=0.02):
        ...
        precursors = np.fromiter(
            (x[1] for x in self.peptides), dtype=np.float, count=len(self.peprec)
        )
        ...
        with engine.connect() as connection:
            ...
            for end in gaps:
                for spec in connection.execute(
                    ...
                    .select_from(tables.spec.join(tables.specfile))
                    .where(tables.spec.c.pepmass > self.peptides[start][1] - max_error)
                    .where(tables.spec.c.pepmass < self.peptides[end][1] + max_error)
                    ...
                    for spec_id, mz, pred_peaks in self.peptides[start : end + 1]:
                        ...
                        if match_mzs(spec.mzs, pred_peaks, max_error=max_error):
                            yield (
                                spec_id,
                                spec.filename,
                                {"params": {"title": spec.spec_id}, "m/z array": spec.mzs},
                            )
                start = end + 1
