...
# Run ms2pipC to predict peak intensities from a PEPREC file (ETD model)
call(['python', '../ms2pipC.py', 'test.PEPREC', '-c', 'config_ETD.file'])
call(['ls'])
test_data = pd.read_csv('test.PEPREC_predictions.csv')
# Load target values & peptide file
target_data = pd.read_csv('target_ETD.PEPREC_predictions.csv')
