...
def load_data(vector_filename, ion_type):
    ...
    if vector_filename.split('.')[-1] == 'pkl':
        ...
        exit(1)
    print("{} contains {} feature vectors".format(vector_filename, len(vectors)))

    # Extract targets for given ion type
    target_names = list(vectors.columns[vectors.columns.str.contains('targets')])
    if 'targets{}'.format(ion_type) not in target_names:
...
def get_best_params(df, params_grid):
    params = {}
    best = df[df['test-mae-mean'] == df['test-mae-mean'].min()]
...
def gridsearch(xtrain, params, params_grid):
    ...
    for param_overrides in combinations:
        ...
        tmp = xgb.cv(
            params, xtrain, nfold=5, num_boost_round=200, early_stopping_rounds=25,
            verbose_eval=10, metrics=('mae', 'rmse'),
            # feval=evalerror_pearson, maximize=True
        )
...
def main():
    ...
    print(params)
    bst = xgb.train(
        params, xtrain, 500, evallist, early_stopping_rounds=25, 
        # feval=evalerror_pearson, maximize=True
    )
