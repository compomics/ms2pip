...
# Run ms2pipC to predict peak intensities from a PEPREC file (iTRAQ Phospho model)
call(['python', '../ms2pipC.py', 'test.PEPREC', '-c', 'config.file', '-i', '-p'])
call(['ls'])
test_data = pd.read_csv('test.PEPREC_predictions.csv')
# Load target values & peptide file
target_data = pd.read_csv('target_iTRAQ-Phospho.PEPREC_predictions.csv')
