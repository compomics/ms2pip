...
# Run ms2pipC to predict peak intensities from a PEPREC file (CID model)
call(['python', '../ms2pipC.py', 'test.PEPREC', '-c', 'config_CID.file'])
call(['ls'])
test_data = pd.read_csv('test.PEPREC_predictions.csv')
# Load target values & peptide file
target_data = pd.read_csv('target_CID.PEPREC_predictions.csv')
